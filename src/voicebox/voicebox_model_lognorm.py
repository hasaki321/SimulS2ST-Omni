...
from src.model.llama_nar import DiffLlama
import torch.nn.functional as F
from tqdm import trange

def _lognormal_band_weights(n_mels: int, device: torch.device) -> torch.Tensor:
    """
    根据对数正态拟合的能量-频带分布计算每 mel bin 的加噪权重。
    能量大的频带权重大（加更多噪声），能量小的权重小，使各频带信噪比大致一致。
    返回 shape [n_mels]，归一化使平均权重为 1。
    """
    # 固定参数（基于 mel 频谱能量分布拟合）
    A, s, scale, B = 76.6, 1.74, 160.0, 0.391

    x = torch.arange(n_mels, dtype=torch.float32, device=device) + 1.0
    ln_x = torch.log(x)
    ln_scale = math.log(scale)
    pdf = (1.0 / (s * x * math.sqrt(2 * math.pi))) * torch.exp(
        -((ln_x - ln_scale) ** 2) / (2 * s * s)
    )
    energy = A * pdf + B
    energy = torch.clamp(energy, min=1e-6)
    return energy / energy.mean()  # [n_mels]
...
class VoiceBox(nn.Module):
    ...
    def reverse_diffusion(
        ...
        w = _lognormal_band_weights(self.mel_dim, cond.device).to(z.dtype).view(1, 1, -1)
        z = z * w
        print("Applying lognormal band weights to noise.")
