...
# Detect transformers version by checking LlamaAttention.forward signature
# New API (4.56+): forward(self, hidden_states, position_embeddings, attention_mask, ...)
# Old API (4.46):  forward(self, hidden_states, attention_mask, position_ids, ...)
def _check_new_attention_api():
    """Check if transformers uses new LlamaAttention API (position_embeddings as required arg)."""
    try:
        from transformers.models.llama.modeling_llama import LlamaAttention
        sig = inspect.signature(LlamaAttention.forward)
        params = list(sig.parameters.keys())
        # New API has position_embeddings as 3rd parameter (after self, hidden_states)
        # Old API has attention_mask as 3rd parameter
        return "position_embeddings" in params and params.index("position_embeddings") == 2
    except:
        return False

_NEW_ATTENTION_API = _check_new_attention_api()
...
class LlamaNARDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # For new API compatibility: create rotary embedding if self_attn doesn't have one
        # or if we need to compute position_embeddings ourselves
        self._head_dim = config.hidden_size // config.num_attention_heads
        self._rotary_emb = None  # Lazy init

    def _get_position_embeddings(self, hidden_states: torch.Tensor, position_ids: torch.Tensor):
        """Compute position embeddings (cos, sin) for new transformers API."""
        if self._rotary_emb is None:
            # Lazy init rotary embedding
            from transformers.models.llama.modeling_llama import LlamaRotaryEmbedding
            # Get config from self_attn
            config = self.self_attn.config if hasattr(self.self_attn, 'config') else None
            if config is not None:
                self._rotary_emb = LlamaRotaryEmbedding(config=config).to(hidden_states.device)
            else:
                # Fallback: create with default params
                self._rotary_emb = LlamaRotaryEmbedding(
                    dim=self._head_dim,
    ...
    def forward(
        ...
        # Self Attention - handle API differences between transformers versions
        if _NEW_ATTENTION_API:
            # New API (4.56+): needs position_embeddings instead of position_ids
            if position_ids is None:
                batch_size, seq_len = hidden_states.shape[:2]
                position_ids = torch.arange(seq_len, device=hidden_states.device).unsqueeze(0).expand(batch_size, -1)

            position_embeddings = self._get_position_embeddings(hidden_states, position_ids)

            # New API returns (hidden_states, attn_weights) - no present_key_value in tuple
            ...
            present_key_value = None  # New API handles cache differently
        else:
            # Old API (4.46): uses position_ids
...
class DiffLlama(LlamaModel):
    def __init__(
        ...
        # Create default config with proper settings for new transformers
        if config is None:
            config = LlamaConfig(
                vocab_size=0,
                hidden_size=256,
        ...
        # Create config with proper attention implementation for new transformers
        layer_config = LlamaConfig(
            hidden_size=hidden_size,
            num_attention_heads=num_heads,
            num_key_value_heads=num_heads,  # Required for new transformers
            max_position_embeddings=4096,
            intermediate_size=hidden_size * 4,
            _attn_implementation="eager",  # Required for new transformers (4.56+)
        )
...
#     x = torch.randn(2, 200, 100)
#     diffusion_step = torch.randint(0, 200, (2,))
#     cond = torch.randn(2, 200, 1024)
#     x_mask = torch.ones(2, 200).bool()
